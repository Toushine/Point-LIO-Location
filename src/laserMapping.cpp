    ...
    if (sync_packages(Measures)) {
        ...
        if (imu_en) {
...

          {
            while (Measures.lidar_beg_time >
                   imu_next.header.stamp
                       .toSec())  // if it is needed for the new map?
            {
              imu_deque.pop_front();
              if (imu_deque.empty()) {
                break;
          ...
          for (k = 0; k < time_seq.size(); k++) {
            ...
            if (imu_en && !imu_deque.empty()) {
              ...
              while (imu_comes) {
                ...
                /*** covariance update ***/
                double dt =
                    imu_next.header.stamp.toSec() - time_predict_last_const;
                kf_output.predict(dt, Q_output, input_in, true, false);
                time_predict_last_const =
                    imu_next.header.stamp.toSec();  // big problem

                {
                  double dt_cov =
                      imu_next.header.stamp.toSec() - time_update_last;
            ...
            while (imu_next.header.stamp.toSec() > time_current &&
              ...
              if (!is_first_frame) {
                ...
                if (dt_cov > 0.0) {
                  // kf_input.predict(dt_cov, Q_input, input_in, false, true);
                  time_update_last =
                      imu_next.header.stamp.toSec();  // time_current;
                }
                // kf_input.predict(dt, Q_input, input_in, true, false);
      ...
      if (runtime_pos_log) {
        ...
        printf(
            ...
            "total: %0.6f icp: %0.6f propogate: %0.6f \n",
